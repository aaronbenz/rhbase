...
###Test it out
Provided HBase and rhbase are now installed correctly and is running, the code below should run successfully.
```{r}
require(rhbase)
library(data.table)
hb.init()
...
###HBase Table Design
It's important that the design of the HBase table suites the desired query pattern(s). A NoSQL Columnar frame of mind is always ***Design Your Tables For Your Query Pattern***. Unlike a relational store, each table that is built is traditionally designed for one single type of query pattern (document stores like Solr or Elastic Search can offer a backwards indexing solution. Overall, this can make a data modelling experience "simpler" in concept). This frame of mind implies that:

  * Joins are BAD (Spark does add some additional capability for this, and will probably only continue to grow in the future)
...
Currently the data in its raw format is **by variable by vehicle by date** and  it is "de-duplicated" for compression purposes. Additionally, because this is archived data, additionally compresstion to a binary blob(byte array) is achievable, drastically reducing the size of the data set while immensely increasing the speed at which it is retrieved (because its just in one blob as opposed to many cells). However, the approache should be cautious of the memory limitations that might exist; the size and number of blobs has to be manageable. That is, data needs to be retrieved in manageable partitions that contain all of the neccessary variables to perform a fuel usage calculation. With those concepts in mind, the query pattern should be able to take on specific airports and date ranges, along with whatever variables that are desired. Thus:
...
Whats great about this is that you can easily parallel this operation across multiple cores/nodes/clusters because we broke it down in intervals (airport and date). Check out the parallel package or even the rmr2 package for more details of how to do that. 
