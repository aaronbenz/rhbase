...
###HBase Frame of Mind
  ...
  - Within a table are **column families**, which is basically just a subgroup of your table. Best practice is to limit the number and size of these. So, if you are new to HBase, just pretend you only have 1 as in many cases that is all that is necessary. 
  ...
  - Data modeling: **Based off query patterns and stored directly**. Cross-table jogins are a BAD thing (Spark can help with this, but that does not mean you should design to do any kind of joins). Essentially you are sacrificing complex querying for huge speed gains.
